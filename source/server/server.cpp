...
public:
    ...
    void check_heartbeat() {
        ...
        { std::shared_lock<std::shared_mutex> lk(rw_mtx);
          for (auto& p : connections_) { int fd = p.first; auto& c = p.second;
              if (duration_cast<seconds>(now - c->last_active).count() > 30) tc.push_back(fd); } }
        for (int fd : tc) { std::cout << "fd=" << fd << " timeout\n"; remove_connection(fd); }
    }
    void remove_connection(int fd) { if (connections_.count(fd)) connections_.erase(fd); close(fd); }
private:
    void handle_message(ClientConn& conn, const Message& msg) {
        if (msg.type == "PING") { queue_send(conn, {"PONG","Server",msg.sender,""}); return; }
        if (msg.type == "LOGIN") { conn.username = msg.sender; conn.state = ClientConn::LOGGED_IN; queue_send(conn, {"LOGIN_OK","Server",msg.sender,"welcome"}); std::cout << "User " << msg.sender << " logged in\n"; return; }
        ...
        if (msg.type == "PRIVATE") { for (auto& p : connections_) if (p.second->username == msg.target && p.second->state == ClientConn::LOGGED_IN) { queue_send(*p.second, msg); return; } queue_send(conn, {"SYS","Server",msg.sender,"user offline"}); }
    }
};
...
int main() {
    ...
    ThreadPool pool(4); ChatServer server;
    auto lhb = steady_clock::now();
    std::cout << "Server started on port 8888\n";
    while (true) {
        int n = epoll_wait(ep, evs, MX, 1000);
        if (n < 0) { if (errno == EINTR) continue; break; }
        auto now = steady_clock::now(); if (duration_cast<seconds>(now - lhb).count() >= 10) { server.check_heartbeat(); lhb = now; }
        for (int i = 0; i < n; i++) {
            int fd = evs[i].data.fd;
            if (fd == sfd) {
                while (true) { int cfd = accept(sfd, nullptr, nullptr); if (cfd < 0) { if (errno == EAGAIN||errno == EWOULDBLOCK) break; break; } int cf = fcntl(cfd, F_GETFL, 0); fcntl(cfd, F_SETFL, cf|O_NONBLOCK); auto cn = std::make_shared<ClientConn>(); cn->fd = cfd; cn->last_active = system_clock::now(); server.connections_[cfd] = cn; struct epoll_event cev{}; cev.events = EPOLLIN|EPOLLET; cev.data.fd = cfd; epoll_ctl(ep, EPOLL_CTL_ADD, cfd, &cev); std::cout << "New fd=" << cfd << "\n"; }
            } else {
                auto it = server.connections_.find(fd); if (it == server.connections_.end()) { epoll_ctl(ep, EPOLL_CTL_DEL, fd, nullptr); continue; }
                if (evs[i].events & (EPOLLHUP|EPOLLERR)) { server.remove_connection(fd); continue; }
                if (evs[i].events & EPOLLIN) server.on_readable(*it->second);
                it = server.connections_.find(fd); if (it == server.connections_.end()) continue;
                if (!it->second->write_buffer.empty()) { struct epoll_event cev{}; cev.events = EPOLLIN|EPOLLOUT|EPOLLET; cev.data.fd = fd; epoll_ctl(ep, EPOLL_CTL_MOD, fd, &cev); }
                if (evs[i].events & EPOLLOUT) { server.do_write(*it->second); it = server.connections_.find(fd); if (it != server.connections_.end() && it->second->write_buffer.empty()) { struct epoll_event cev{}; cev.events = EPOLLIN|EPOLLET; cev.data.fd = fd; epoll_ctl(ep, EPOLL_CTL_MOD, fd, &cev); } }
            }
        }
    }
