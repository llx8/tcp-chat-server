...
struct ClientConn { int fd = -1; std::string username; std::string recv_buffer; std::string write_buffer; system_clock::time_point last_active; enum State { CONNECTED, LOGGED_IN } state = CONNECTED; };
class ChatServer {
public:
    RedisCache* redis_ = nullptr;
    ...
    void on_readable(ClientConn& conn) {
        ...
        while (conn.recv_buffer.size() >= 4) { uint32_t nl; std::memcpy(&nl, conn.recv_buffer.data(), 4); uint32_t pl = ntohl(nl); if (conn.recv_buffer.size() < 4 + pl) break; std::string p = conn.recv_buffer.substr(4, pl); conn.recv_buffer.erase(0, 4 + pl); handle_message(conn, Message::deserialize(p)); }
        if (!conn.write_buffer.empty()) do_write(conn);
    }
    void do_write(ClientConn& conn) { while (!conn.write_buffer.empty()) { ssize_t s = send(conn.fd, conn.write_buffer.data(), conn.write_buffer.size(), MSG_NOSIGNAL); if (s < 0) { if (errno == EAGAIN || errno == EWOULDBLOCK) break; remove_connection(conn.fd); return; } if (s == 0) break; conn.write_buffer.erase(0, s); } }
    void queue_send(ClientConn& conn, const Message& msg) { std::string p = msg.serialize(); uint32_t nl = htonl(p.size()); conn.write_buffer.append(reinterpret_cast<char*>(&nl), 4); conn.write_buffer.append(p); }
    void check_heartbeat() { auto now = system_clock::now(); std::vector<int> tc; { std::shared_lock<std::shared_mutex> lk(rw_mtx); for (auto& p : connections_) { int fd = p.first; auto& c = p.second; if (duration_cast<seconds>(now - c->last_active).count() > 30) tc.push_back(fd); } } for (int fd : tc) { std::cout << "fd=" << fd << " timeout\n"; remove_connection(fd); } }
...
private:
    void handle_message(ClientConn& conn, const Message& msg) {
        ...
        if (msg.type == "REGISTER") { std::string u = msg.sender; if (!db_->register_user(u, msg.content)) { queue_send(conn, {"REG_FAIL","Server",u,"user exists"}); return; } queue_send(conn, {"REG_OK","Server",u,"register success"}); std::cout << "User " << u << " registered\n"; return; }
        ...
        if (msg.type == "PRIVATE") { std::shared_lock<std::shared_mutex> lk(rw_mtx); auto it = username_to_fd_.find(msg.target); if (it != username_to_fd_.end()) { queue_send(*connections_[it->second], msg); } else { redis_->push_offline(msg.target, msg); queue_send(conn, {"SYS","Server",msg.sender,"target offline, message saved"}); } return; }
    }
};
int main() {
    ...
    RedisCache redis("127.0.0.1", 6379); if (!redis.connect()) { std::cerr << "Redis init failed\n"; return 1; }
    Database db("127.0.0.1", "root", "", "chat"); if (!db.connect()) { std::cerr << "MySQL init failed\n"; return 1; }
    ChatServer server; server.redis_ = &redis; server.db_ = &db;
