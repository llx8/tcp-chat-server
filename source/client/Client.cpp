...
int main() {
    ...
    std::cout << "========= Commands =========\n";
    std::cout << "/register <user> <pass>  - 注册\n";
    std::cout << "/login <user> <pass>     - 登录\n";
    std::cout << "/all <msg>               - 群发\n";
    std::cout << "/to <user> <msg>         - 私聊\n";
    std::cout << "/quit                    - 退出\n";
    std::cout << "============================\n> ";
    std::string line; while (running && std::getline(std::cin, line)) { std::string in = trim(line); if (in.empty()) { std::cout << "> "; continue; }
        ...
        else if (in.find("/to ") == 0) { std::stringstream ss(in); std::string _,t,m; ss >> _ >> t; std::getline(ss,m); m = trim(m); if (!t.empty() && !m.empty()) Connection::send_frame(fd, {"PRIVATE",my_username,t,m}); else std::cout << "用法: /to <user> <msg>\n> "; }
        else if (in.find("/all ") == 0) { std::string m = trim(in.substr(5)); if (!m.empty()) Connection::send_frame(fd, {"CHAT",my_username,"ALL",m}); else std::cout << "用法: /all <msg>\n> "; }
